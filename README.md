...
ArcIntelligence `pr-reviewer` is an AI-based code reviewer and summarizer for
...
- [Overview](#overview)
- [Professional Version of ARC Intelligence](#professional-version-of-arcintelligence)
- [Reviewer Features](#reviewer-features)
- [Install instructions](#install-instructions)
- [Conversation with arc-intelligence](#conversation-with-arcintelligence)
- [Examples](#examples)
...
jobs:
  review:
    ...
    steps:
      - uses: bhupesh-sf/pr-reviewer@latest
        env:
...
```yaml
system_message: |
  You are `@arcintelligenceai` (aka `github-actions[bot]`), a language model
  trained by OpenAI. Your purpose is to act as a highly experienced
  DevRel (developer relations) professional with focus on cloud-native
  infrastructure.

  Company context -
  arcintelligence is an AI-powered Code reviewer.It boosts code quality and cuts manual effort. Offers context-aware, line-by-line feedback, highlights critical changes,
...
## Conversation with arcintelligence

You can reply to a review comment made by this action and get a response based
on the diff context. Additionally, you can invite the bot to a conversation by
tagging it in the comment (`@arcintelligenceai`).

Example:

> @arcintelligenceai Please generate a test plan for this file.
...
jobs:
  review:
    runs-on: ubuntu-latest
    steps:
      - uses: arcintelligenceai/ai-pr-reviewer@latest
        env:
          GITHUB_TOKEN: ${{ secrets.GITHUB_TOKEN }}
          OPENAI_API_KEY: ${{ secrets.OPENAI_API_KEY }}
        with:
